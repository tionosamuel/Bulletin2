...
class CreatePayments extends Component
{
 ...
 $montantScolarite = $query->montant;
   ...
   //Verifier que le montant total de la scolarité n'est pas inférieur au montant déja payé + le montant du paiement en cour
    ...
    $this->error = 'Attention. Il reste a payer ' . $montantScolarite - $totalPaid . ' F/CFA';
    $this->haveError = true;
      }
    ...
    try {
          //recuperer l'id de l  cl sse de lélève
          $getClasseQuery = Attribution::where('student_id', '=',
          $this->student_id)->where('school_year_id',
          $activeSchoolYear->id)->first();
    ...
    $payment->save();
       ...
       return redirect()->route('payments')->with('success', 'paiement de la scolarité Effectué ');
    } catch (Exception $e) {
